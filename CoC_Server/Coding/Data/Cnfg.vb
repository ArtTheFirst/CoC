...
Public Class Cnfg
    ...
    '//Enumerators
    Public Shared Actv As Integer = 31
    Public Shared InActv As Integer = 41
    ...
    Shared Sub loadSetting(scopeName, scopeValue)
        Try
            ...
            Dim cm As New NpgsqlCommand("UPDATE settingsTbl SET cScope=@cScope WHERE cName='" & scopeName & "'",
                                           c)
            Dim f As NpgsqlDbType = NpgsqlDbType.Unknown
            With cm
                If TypeOf (scopeValue) Is Integer Then
                    f = NpgsqlDbType.Integer
                Else
                    f = NpgsqlDbType.Varchar
                End If
                .Parameters.Add(New NpgsqlParameter("@cScope", f)).Value = scopeValue
                .ExecuteNonQuery()
    ...
    Public Shared Sub Reset()
        ...
        loadSetting("subAdminModifyCashier", InActv)
        loadSetting("subAdminModifyInvoices", InActv)
        loadSetting("CompanyPhone", "")
        loadSetting("CompanyType", "")
        loadSetting("CompanyEmail", "")
        loadSetting("AdminLimit", 1)
        loadSetting("ClientLimit", 1)
        loadSetting("AllowSubAdmin", InActv)
        loadSetting("PerformBackups", InActv)
        loadSetting("subAdminModifyOwnKey", InActv)
        loadSetting("subAdminModifyCompany", InActv)
        loadSetting("subAdminModifySecurity", InActv)
        loadSetting("subAdminCategoryControl", InActv)
        loadSetting("subAdminReportControl", InActv)
        loadSetting("subAdminBackupControl", InActv)
        loadSetting("backupTime", "")
        loadSetting("lastBackup", "")
        loadSetting("backupModeAuto", InActv)
        loadSetting("subAdminModifyFeatures", InActv)
        loadSetting("subAdminExportReports", InActv)
        loadSetting("backupPath", "")
        loadSetting("IsFirstTimeUse", Actv)
