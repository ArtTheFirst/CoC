...
Public Class Distributors_Ctrl
    ...
    Sub clearFields()
        ...
        t_Company.Clear()
        t_Address.Clear()
        c_Distributors.Text = Nullify
        c_PreferredCurrency.Text = Nullify
        c_PreferredPayment.Text = Nullify
        dId = String.Concat("DR", Date.Now.Year, HexLongSerialId)
        l_DistributorId.Text = String.Concat("distributor id ", dId)
    ...
    Sub initializeId()
        If Not IdXst() Then
            xQry("INSERT INTO clDistributors (SerialId) VALUES ('" & dId & "')")
            OnEdit = True
        End If
    ...
    Function IdXst() As Boolean
        Return xXst("SerialId", "clDistributors", dId) = xData.Exists
    End Function
    ...
    Private Sub c_Distributors_SelectedIndexChanged(sender As Object, e As EventArgs) Handles c_Distributors.SelectedIndexChanged
        dId = Qry("SELECT SerialId FROM clDistributors WHERE clName='" & c_Distributors.Text & "'")

        If dId = Nothing Or dId = NullVal Then Exit Sub

        If Not IdXst() Then Exit Sub

        tbl = xRead("SELECT * FROM clDistributors WHERE SerialId='" & dId & "'", "clDistributors")

        If tbl Is Nothing Or tbl.Rows.Count < 1 Then clearFields() : Exit Sub
        clearFields()

        For Each row In tbl.Rows
            l_DistributorId.Text = String.Concat("distributor id ", row(1).ToString)
            c_Distributors.Text = row(2).ToString
    ...
    Private Sub btnCreate_Click(sender As Object, e As EventArgs) Handles btnCreate.Click
        If c_Distributors.Text = NullVal Then MsgBox("You have not provided needed data!", MsgBoxStyle.Information) : Exit Sub
        Try
            With c
                .Open()
            End With
            Dim cm As New SqlCommand("UPDATE clDistributors SET clName=@clName,clCompany=@clCompany," &
                                                                                                           "clAddress=@clAddress,clPreferredPay=@clPreferredPay," &
                                                                                                           "clPreferredCurrency=@clPreferredCurrency,clPurchasePower=@clPurchasePower " &
                                                                                       "WHERE SerialId='" & dId & "'",
                                       c)
            With cm
                .Parameters.Add(New SqlParameter("@clName", SqlDbType.Text)).Value = c_Distributors.Text
    ...
    Private Sub btnDelete_Click(sender As Object, e As EventArgs) Handles btnDelete.Click
        ...
        Dim sS As String = String.Concat("Delete ",
                                                                xQry("DELETE FROM clDistributors WHERE SerialId='" & dId & "'"))
        clearFields()
