...
Public Class ClientDetails
    ...
    Sub uploadData()
        Try
            ...
            If _Data = _AccessMode.IsUpload Then
                If Not xlpth_Clients.Exists Then xlpth_Clients.Create()

                c_c.Open()
                Dim tSQL As String = "SELECT * FROM [c_Data$]"
                Dim cmd As New OleDbCommand(tSQL, c_c)
            ...
            ElseIf _Data = _AccessMode.IsDownload Then
                tbl = xRead("SELECT * FROM clCustomers",
                            "clCustomers",
                            c)
            End If
    ...
    Protected Overrides Sub OnLoad(e As EventArgs)
        MyBase.OnLoad(e)
        btnBeginUpload.Text = "BEGIN " & IIf(_Data = _AccessMode.IsDownload, "DOWNLOAD", "UPLOAD")
    ...
    Public Sub LoadItems() Handles btnBeginUpload.Click
        ...
        Try
            If _Save = _Mode.IsSave Then
                ...
                xQry("DELETE FROM clCustomers", c)

            End If
            sql = "INSERT INTO clCustomers (SerialId,clName,clCompany,clOutstanding,clAddress,clPreferredPay," &
            ...
            For Each row As DataGridViewRow In dg_List.Rows
                ...
                With cm
                    ...
                    .Parameters.Add(New NpgsqlParameter("@clCompany", NpgsqlDbType.Varchar)).Value = row.Cells(2).Value
                    .Parameters.Add(New NpgsqlParameter("@clAddress", NpgsqlDbType.Varchar)).Value = row.Cells(3).Value
                    .Parameters.Add(New NpgsqlParameter("@clPreferredPay", NpgsqlDbType.Varchar)).Value = row.Cells(4).Value
                    .Parameters.Add(New NpgsqlParameter("@clPreferredCurrency", NpgsqlDbType.Varchar)).Value = row.Cells(5).Value
