...
Public Class PayMethod
    ...
    Sub loadList()
        Dim tbl As DataTable = xRead("SELECT * FROM clPayMethods", "clPayMethods",)
        dg_List.Rows.Clear()
        t_Name.Clear()
    ...
    Private Sub _KeyDownClose(sender As Object, e As KeyEventArgs) Handles Me.KeyDown
        ...
        ElseIf e.KeyCode = Keys.F1 Then
            ...
            Dim result As String = xQry("INSERT INTO clPayMethods (SerialId,clName) VALUES ('" & SerialId() & "','" & t_Name.Text & "')",)
            MsgBox(result)
            loadList()
    ...
    Private Sub dgList_KeyDown(sender As Object, e As KeyEventArgs) Handles dg_List.KeyDown
        ...
        If e.KeyData = Keys.Delete Then
            Try
                ...
                result = xQry("DELETE FROM clPayMethods WHERE SerialId='" & nDR.Cells(0).Value & "'",)
                MsgBox(result)
            Catch ex As Exception
            Finally
                loadList()
        ...
        ElseIf e.KeyCode = Keys.Enter Then
            ...
            For Each row As DataGridViewRow In dg_List.Rows
                If Not row.Cells(0).Value = "" Or Not row.Cells(1).Value = "" Then
                    result = xQry("UPDATE clPayMethods SET clName='" & row.Cells(1).Value &
