Public Class Banks
    ...
    Sub loadList()
        Dim tbl As DataTable = xRead("SELECT * FROM clBanks", "clBanks",)
        dg_List.Rows.Clear()
        t_Name.Clear()
    ...
    Private Sub _KeyDownClose(sender As Object, e As KeyEventArgs) Handles Me.KeyDown
        ...
        ElseIf e.KeyCode = Keys.F1 Then
            ...
            Dim result As String = xQry("INSERT INTO clBanks (SerialId,clName) VALUES ('" & SerialId() & "','" & t_Name.Text & "')",)
            MsgBox(result)
            loadList()
    ...
    Private Sub dgList_KeyDown(sender As Object, e As KeyEventArgs) Handles dg_List.KeyDown
        ...
        If e.KeyData = Keys.Delete Then
            Try
                ...
                result = xQry("DELETE FROM clBanks WHERE SerialId='" & nDR.Cells(0).Value & "'",)
                MsgBox(result)
            Catch ex As Exception
            Finally
                loadList()
        ...
        ElseIf e.KeyCode = Keys.Enter Then
            ...
            For Each row As DataGridViewRow In dg_List.Rows
                If Not row.Cells(0).Value = "" Or Not row.Cells(1).Value = "" Then
                    result = xQry("UPDATE clBanks SET clName='" & row.Cells(1).Value &
